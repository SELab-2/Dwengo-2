...
export function createParamsExtractor<T extends ServiceParams>(
  ...
  return (req: Request): T => {
    ...
    const paramInfos = getConstructorParamInfo(ParamsClass);
    const args: unknown[] = [];
    ...
    for (const paramInfo of paramInfos) {
      ...
      if (paramName === 'id' && !value && pathParams.id) {
        value = pathParams.id;
      } else if (paramName === 'parentId' && !value && pathParams.idParent) {
        value = pathParams.idParent;
      }

      // Apply custom transformation if provided
      if (value !== undefined && transformations[paramName]) {
        value = transformations[paramName](value);
      }
      ...
      if (paramInfo.required && value === undefined) {
        throw {
          code: 'BAD_REQUEST',
          message: `Required parameter '${mappedName}' is missing`
        };
      }
 ...
 */
function getConstructorParamInfo<T extends ServiceParams>(ctor: ServiceParamsConstructor<T>): ParamInfo[] {
  // Get the constructor as a string
  ...
  return params
    ...
    .map(param => {
      ...
      // Remove modifiers (protected, private, etc.)
      const withoutModifiers = trimmed.replace(/^protected\s+|^private\s+|^public\s+|^readonly\s+/, '');

      // Check for optional parameter (has ? or =)
      const isOptional = withoutModifiers.includes('?') || withoutModifiers.includes('=');
