...
export class ChallengeManager {
    ...
    public async verifyChallenge(userId: UUID, signedChallenge: string, userType: UserType): Promise<boolean> {
        try {
            ...
            const publicKey = createPublicKey({ key: user.passwordHash, format: "pem", type: "spki" });

            // Convert signedChallenge from base64 string to Buffer
            return crypto.verify(null, Buffer.from(challenge), publicKey, Buffer.from(signedChallenge, "base64"));
        } catch (error) {
