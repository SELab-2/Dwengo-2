...
export class DatasourceTaskTypeORM extends DatasourceTypeORM {
    ...
    public async create(task: Task): Promise<Task> {
        ...
        const assignmentModel = await datasource.getRepository(AssignmentTypeORM)
            .findOne({
    ...
    public async getById(taskId: string): Promise<Task> {
        ...
        const taskModel = await datasource.getRepository(TaskTypeORM)
            .findOne({
                where: { id: taskId },
    ...
    public async getByAssignmentId(assignmentId: string, step?: number): Promise<Task[]> {
        ...
        if (step) {
            const taskModels = await datasource.getRepository(TaskTypeORM)
                .findBy({ assignment: assignmentModel, step: step });
            return taskModels.map(taskModel => taskModel.toEntity());
        }

        const taskModels = await datasource.getRepository(TaskTypeORM)
            .findBy({ assignment: assignmentModel });
        return taskModels.map(taskModel => taskModel.toEntity());
    }
