...
const mockStudentRepository = {
  getStudentByEmail: jest.fn().mockResolvedValue(false), // Simulate that email is not in use
...
describe("CreateStudent", () => {
  ...
  test("Should throw error if email is already in use", async () => {
    mockStudentRepository.getStudentByEmail.mockResolvedValue(true);
    ...
    expect(mockStudentRepository.getStudentByEmail).toHaveBeenCalledWith("test@example.com");
  });
