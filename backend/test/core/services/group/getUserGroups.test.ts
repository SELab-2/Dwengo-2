...
describe('GetUserGroups', () => {
    ...
    test('Should retrieve all groups for a given user successfully', async () => {
        ...
        const result = await getUserGroups.execute({id});

        expect(result).toEqual({ groups: groups.map(group => group.toObject()) });
        expect(mockGroupRepository.getByUserId).toHaveBeenCalledWith(id);
    });
    ...
    test('Should throw a DatabaseError if retrieval fails', async () => {
        ...
        await expect(getUserGroups.execute({id})).rejects.toThrow(DatabaseError);
