import { GetUserGroups, GetUserGroupsParams } from '../../../../src/core/services/group/getUserGroups';
...
describe('GetUserGroups', () => {
    ...
    test('Should retrieve all groups for a given user successfully', async () => {
        const inputParams = new GetUserGroupsParams("user-123");
        const groups = [
        ...
        const result = await getUserGroups.execute(inputParams);
    ...
    test('Should throw a DatabaseError if retrieval fails', async () => {
        ...
        await expect(getUserGroups.execute(inputParams)).rejects.toThrow(DatabaseError);
        expect(mockGroupRepository.getByUserId).toHaveBeenCalledWith("user-123");
    });
});
