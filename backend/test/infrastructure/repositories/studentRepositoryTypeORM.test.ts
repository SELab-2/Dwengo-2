...
describe("StudentRepositoryTypeORM", () => {
    ...
    beforeEach(() => {
        datasourceMock = {
            getDatasourceStudent: jest.fn(), // TODO: should work?
    ...
    test("getStudentByFirstName", async () => {
        // Call function from repository
        const returnStudent: Student|null = await datasourceStudent.getStudentByFirstName(student.first_name);

        expect(datasourceStudent.getStudentByFirstName).toHaveBeenCalledTimes(1);
        expect(datasourceStudent.getStudentByFirstName).toHaveBeenCalledWith(student.first_name);
        expect(returnStudent).toEqual(student);
    });

    test("getStudentByLastName", async () => {
        // Call function from repository
        const returnStudent: Student|null = await datasourceStudent.getStudentByLastName(student.family_name);

        expect(datasourceStudent.getStudentByLastName).toHaveBeenCalledTimes(1);
        expect(datasourceStudent.getStudentByLastName).toHaveBeenCalledWith(student.family_name);
        expect(returnStudent).toEqual(student);
    });
